...
const Navbar = ({ links, typeOfLink, main }: { links: { text: string, url: string }[], typeOfLink: string, main: boolean }) => {
    ...
    return (
        <>
            <nav className={`flex justify-between w-full items-center py-6 z-[101] relative `}>
                <div className='flex items-center'>
                    <Link to={'/'}>
                        <h2 className='text-white uppercase text-2xl font-anurati'>P<span className='text-secondary'>R</span></h2>
                    </Link>

                    <ul className='text-white font-kodeMono text-[1.070rem] md:flex hidden uppercase rounded-[10px] px-8 py-4 items-center ml-[2rem]'>
                ...
                <div className={`z-[100] ${main ? 'hidden' : 'hidden md:block'}`}>
            ...
            <ul className={`${toggle ? 'left-0' : 'left-[-100%]'} text-white font-kodeMono text-[1.2rem] fixed bottom-0 w-full md:hidden bg-black transition-all duration-500 text-center left-0 z-[100] h-full`}>
